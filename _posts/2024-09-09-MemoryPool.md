...
其实从总的来看，内存池还有一个好处，提前分配好内存可以把一部分时间性能消耗大的操作移动到程序刚启动的时候，而程序后续运行期间则可以变得更加顺滑。特别是对于一些使用 gc 管理内存的编程语言（比如 go、java、python），程序运行期间内存的申请/释放的时间消耗会有更多的不确定性。

这个特点在高性能服务器、游戏引擎等实现中尤为重要。就以游戏引擎为例，你是希望玩游戏的时候开头小卡一下，但是后面爽快畅玩，还是希望你玩游戏的时候从头卡到尾呢？答案应该很明显了吧。
